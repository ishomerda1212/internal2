...
import { useCreateCompanyCar } from '../../hooks/useCompanyCars'
...
export const CompanyCarForm: React.FC<CompanyCarFormProps> = ({
  onClose,
  onSuccess
}) => {
  const { data: organizations = [] } = useOrganizations()
  const createCompanyCar = useCreateCompanyCar()
  ...
  const onSubmit = async (data: FormData) => {
    try {
      ...
      await createCompanyCar.mutateAsync({
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
      <div className="bg-white rounded-lg shadow-xl w-full max-w-2xl mx-4 max-h-[90vh] overflow-y-auto">
        <div className="flex items-center justify-between p-6 border-b border-gray-200">
          <h2 className="text-lg font-semibold text-gray-900">新規車両登録</h2>
          <button
            onClick={onClose}

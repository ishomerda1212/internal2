-- 階層別組織対応のためのtransfer_historiesテーブル更新
-- 実行前にバックアップを取得してください
...
-- 2. 既存データの移行（現在のorganization_idを第三階層として設定）
...
-- 3. 古いorganization_idカラムを削除（CASCADEで依存関係も削除）
ALTER TABLE transfer_histories 
...
-- 5. employees_with_current_assignmentビューを更新
...
SELECT 
  ...
  th.start_date,
  CASE 
    WHEN th.organization_level_1_id IS NULL AND th.organization_level_2_id IS NULL AND th.organization_level_3_id IS NULL 
    THEN '全て未設定'
    ELSE '一部設定済み'
  END as status
FROM transfer_histories th
JOIN employees e ON th.employee_id = e.id
WHERE th.organization_level_1_id IS NULL 
  AND th.organization_level_2_id IS NULL 
  AND th.organization_level_3_id IS NULL; 